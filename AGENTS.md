...
│   ├── agent-factory.py              #   Agent Factory CLI（自然言語 → エージェント自動生成）
...
│   ├── create-subagent.sh            #   新規 Subagent 作成（手動テンプレート）
...
### 4.5 Agent Factory — 自然言語からエージェント自動生成

Agent Factory は自然言語の仕様から高品質なエージェントを完全自動生成する4フェーズパイプライン。
