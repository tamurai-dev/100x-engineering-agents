...
> v1.0 のリファクタリング中。旧名は `100x Engineering Agents`、旧ターム「Duet」は「Duet」に順次置換中。
...
```bash
# 1. クローン（リポジトリ名は順次 duo-agents にリネーム予定）
...
Agent Factory がシングルエージェントを自動生成するように、Duet Factory は Actor-Critic ペアをセットで自動生成する:
